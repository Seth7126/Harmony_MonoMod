...
namespace HarmonyLib
{
    internal static class MethodCreatorTools
    {
        ...
        internal static List<CodeInstruction> GenerateVariableInit(
            this MethodCreator _,
        ...
        internal static CodeInstruction MarkBlock(
            this MethodCreator _,
        ...
        )
        {
            ...
            if (
                allowFirsParamPassthrough
                && patch.ReturnType != typeof(void)
                && parameters.Count > 0
            ...
            foreach (var injection in injections)
            {
                ...
                if (injectionType == InjectionType.Instance)
                {
                    if (originalIsStatic)
                    {
                        ...
                        {
                            // On Mono, ldnull (type O) is incompatible with by-ref parameters
                            // (type &), causing "Invalid IL code ... call 0x0a..." errors.
                            // Use a default-initialized temp local and pass its address instead.
                            var elementType = paramType.GetElementType();
                            var tmpLocal = config.DeclareLocal(elementType);
                            codes.Add(Ldloca[tmpLocal]);
                        }
                        else
                            codes.Add(Ldnull);
                    }
                ...
                {
                    if (
                        config.localVariables.TryGetValue(
                            InjectionType.ArgsArray,
                            out var argsArrayVar
                        )
                    )
                ...
                if (paramRealName.StartsWith(INSTANCE_FIELD_PREFIX, StringComparison.Ordinal))
                {
                    ...
                    else
                    {
                        ...
                        if (fieldInfo is null)
                            throw new ArgumentException(
                                $"No such field defined in class {originalType?.AssemblyQualifiedName ?? "null"}",
                                fieldName
                            );
                    }
                ...
                if (injectionType == InjectionType.State)
                {
                    ...
                    if (
                        config.localVariables.TryGetValue(
                            patch.DeclaringType?.AssemblyQualifiedName ?? "null",
                            out var stateVar
                        )
                    )
                ...
                if (injectionType == InjectionType.Result)
                {
                    ...
                    var ldlocCode =
                        paramType.IsByRef && returnType.IsByRef is false
                            ? OpCodes.Ldloca
                            : OpCodes.Ldloc;
                    if (returnType.IsValueType && paramType == typeof(object).MakeByRefType())
                        ldlocCode = OpCodes.Ldloc;
                    codes.Add(
                        new CodeInstruction(ldlocCode, config.GetLocal(InjectionType.Result))
                    );
                ...
                if (injectionType == InjectionType.ResultRef)
                {
                    ...
                    var resultType = paramType;
                    var expectedTypeRef = typeof(RefResult<>)
                        .MakeGenericType(returnType.GetElementType())
                        .MakeByRefType();
                ...
                if (paramRealName.StartsWith(PARAM_INDEX_PREFIX, StringComparison.Ordinal))
                {
                    ...
                    if (!int.TryParse(val, out argumentIdx))
                        throw new Exception(
                            $"Parameter {paramRealName} does not contain a valid index"
                        );
                ...
                {
                    argumentIdx = patch.GetArgumentIndex(
                        originalParameterNames,
                        injection.parameterInfo
                    );
                    if (argumentIdx == -1)
                    {
                        ...
                        {
                            var delegateConstructor = paramType.GetConstructor([
                                typeof(object),
                                typeof(IntPtr),
                            ]);
                            if (delegateConstructor is not null)
                            {
                                ...
                                if (
                                    methodInfo.IsStatic is false
                                    && harmonyMethod.nonVirtualDelegate is false
                                )
                                {
                                    codes.Add(Dup);
                ...
                var originalParamElementType = originalParamType.IsByRef
                    ? originalParamType.GetElementType()
                    : originalParamType;
                var patchParamType = paramType;
                var patchParamElementType = patchParamType.IsByRef
                    ? patchParamType.GetElementType()
                    : patchParamType;
                var originalIsNormal =
                    originalParameters[argumentIdx].IsOut is false
                    && originalParamType.IsByRef is false;
                var patchIsNormal =
                    injection.parameterInfo.IsOut is false && patchParamType.IsByRef is false;
                var needsBoxing =
                    originalParamElementType.IsValueType
                    && patchParamElementType.IsValueType is false;
                ...
                if (originalIsNormal == patchIsNormal)
                {
                    ...
                    if (needsBoxing)
                    {
                        ...
                        else
                        {
                            ...
                            tmpBoxVars.Add(
                                new KeyValuePair<LocalBuilder, Type>(
                                    tmpBoxVar,
                                    originalParamElementType
                                )
                            );
                        }
                    }
                ...
                else
                {
                    if (originalParamElementType.IsValueType)
                        ...
                        codes.Add(
                            new CodeInstruction(
                                LoadIndOpCodeFor(originalParameters[argumentIdx].ParameterType)
                            )
                        );
                }
            }
        ...
        )
        {
            ...
            return
            [
                .. vars.Select(lvi => creator.config.il.DeclareLocal(lvi.LocalType, lvi.IsPinned)),
            ];
        }
        ...
        internal static IEnumerable<CodeInstruction> CleanupCodes(
            this MethodCreator creator,
        ...
        internal static void EmitCodes(
            this MethodCreator _,
            Emitter emitter,
            List<CodeInstruction> codeInstructions
        )
