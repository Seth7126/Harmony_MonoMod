    ...
    public class CodeMatcher
    {
        ...
        public List<CodeInstruction> Instructions(int count)
        {
            if (Pos < 0 || Pos + count > Length)
                return HandleException<List<CodeInstruction>>(
                    "Cannot retrieve instructions: range is out-of-bounds.",
                    []
                );
        ...
        public List<Label> DistinctLabels(IEnumerable<CodeInstruction> instructions) =>
            [.. instructions.SelectMany(instruction => instruction.labels).Distinct()];
        ...
        public CodeMatcher RemoveInstruction()
        {
            if (IsInvalid)
                return HandleException(
                    "Cannot remove instructions from an invalid position.",
                    this
                );
        ...
        public CodeMatcher RemoveInstructions(int count)
        {
            if (IsInvalid || Pos + count > Length)
                return HandleException(
                    "Cannot remove instructions from an invalid or out-of-range position.",
                    this
                );
        ...
        public CodeMatcher MatchStartForward(params CodeMatch[] matches) =>
            Match(matches, 1, MatchPosition.Start, false);
        ...
        public CodeMatcher PrepareMatchStartForward(params CodeMatch[] matches) =>
            Match(matches, 1, MatchPosition.Start, true);
        ...
        public CodeMatcher MatchEndForward(params CodeMatch[] matches) =>
            Match(matches, 1, MatchPosition.End, false);
        ...
        public CodeMatcher PrepareMatchEndForward(params CodeMatch[] matches) =>
            Match(matches, 1, MatchPosition.End, true);
        ...
        /// <returns>The same code matcher</returns>
        ///
        public CodeMatcher Repeat(
            Action<CodeMatcher> matchAction,
            Action<string> notFoundAction = null
        )
        {
            var count = 0;
